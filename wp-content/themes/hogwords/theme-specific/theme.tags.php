...
if ( !function_exists('hogwords_need_page_title') ) {
	function hogwords_need_page_title() {
		return !is_front_page() && apply_filters('hogwords_filter_need_page_title', true);
	}
}
...
if ( !function_exists('hogwords_show_layout') ) {
	function hogwords_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('hogwords_get_logo_image') ) {
	function hogwords_get_logo_image($type='') {
		$logo_image = '';
		if (hogwords_is_on(hogwords_get_theme_option('logo_retina_enabled')) && hogwords_get_retina_multiplier(2) > 1)
			$logo_image = hogwords_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = hogwords_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return hogwords_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('hogwords_get_header_video') ) {
	function hogwords_get_header_video() {
		$video = '';
		if (apply_filters('hogwords_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (hogwords_check_theme_option('header_video')) {
				$video = hogwords_get_theme_option('header_video');
...
if ( !function_exists('hogwords_show_post_meta') ) {
	function hogwords_show_post_meta($args=array()) {

		if (is_single() && hogwords_is_off(hogwords_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('hogwords_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php hogwords_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('hogwords_filter_get_post_date', hogwords_get_date());
					if (!empty($dt)) {
						?>
						<span class="post_meta_item post_date<?php if (!empty($args['seo'])) echo ' date updated'; ?>"<?php if (!empty($args['seo'])) echo ' itemprop="datePublished"'; ?>><span class="post_meta_item_posted"><?php esc_html_e( 'Posted ', 'hogwords' )?></span><a href="<?php the_permalink(); ?>"><?php echo wp_kses_data($dt); ?></a></span>
						<?php
					}
				...
				// Post author
					...
					if ($author_id > 0) {
						...
						?><span class="post_meta_item_by"><?php esc_html_e( 'by ', 'hogwords' )?></span>
				...
				// Post counters
					...
					if ( ($output = hogwords_get_post_counters($args['counters'])) != '')
						hogwords_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php comments_link(); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(($post_comments === 1) ? esc_html__('Comment', 'hogwords') :  esc_html__('Comments', 'hogwords'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					hogwords_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'hogwords'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'hogwords' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('hogwords_show_post_featured') ) {
	function hogwords_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => hogwords_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: hogwords_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? hogwords_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = hogwords_get_post_image();
				if (!empty($image)) 
					$image = hogwords_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . hogwords_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 hogwords_is_on(hogwords_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$hogwords_attr = hogwords_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($hogwords_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($hogwords_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						hogwords_hovers_add_icons($args['hover']);
					}
					...
					if (($output=hogwords_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						hogwords_show_layout($output);
					...
					if (!empty($image)) {
						...
						hogwords_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = hogwords_get_post_video($post_content, false);
						if (empty($video))
							$video = hogwords_get_post_iframe($post_content, false);
						if ( empty( $video ) ) {
						   $post_content = hogwords_get_post_content();
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = hogwords_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = hogwords_array_get_first( $videos, false );
					...
					// Put audio over the thumb
						...
						$audio = hogwords_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = hogwords_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								// Add author and title
								$media_author = hogwords_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = hogwords_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !hogwords_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php hogwords_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !hogwords_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php hogwords_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								hogwords_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				hogwords_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				hogwords_show_layout($post_info);
...
if ( ! function_exists( 'hogwords_filter_post_content' ) ) {
	function hogwords_filter_post_content( $content ) {
		$content = apply_filters( 'hogwords_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'hogwords_get_post_content' ) ) {
	function hogwords_get_post_content( $apply_filters=false ) {
		global $post;
...
if ( !function_exists('hogwords_get_no_image') ) {
	function hogwords_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = hogwords_get_theme_option( 'no_image' );
			if (empty($img)) $img = hogwords_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('hogwords_add_bg_in_post_nav') ) {
	function hogwords_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = hogwords_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), hogwords_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), hogwords_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'hogwords-main', $css );
	}
}

// Show related posts
if ( !function_exists('hogwords_show_related_posts') ) {
	function hogwords_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : hogwords_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'hogwords');
				?></h3>
...
if ( !function_exists('hogwords_show_portfolio_posts') ) {
	function hogwords_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? hogwords_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = hogwords_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = hogwords_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the hogwords_ajax_get_posts action
if ( !function_exists( 'hogwords_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_hogwords_ajax_get_posts',			'hogwords_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_hogwords_ajax_get_posts',	'hogwords_ajax_get_posts_callback');
	function hogwords_ajax_get_posts_callback() {
		if ( !wp_verify_nonce( hogwords_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			wp_die();
	
		$id = !empty($_REQUEST['blog_template']) ? wp_kses_data(wp_unslash($_REQUEST['blog_template'])) : 0;
		if ((int) $id > 0) {
			hogwords_storage_set('blog_archive', true);
			hogwords_storage_set('blog_mode', 'blog');
			hogwords_storage_set('options_meta', get_post_meta($id, 'hogwords_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => hogwords_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'hogwords');
		}
...
// Show pagination
if ( !function_exists('hogwords_show_pagination') ) {
	function hogwords_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = hogwords_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'hogwords' ),
				'next_text' => esc_html__( '>', 'hogwords' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'hogwords' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'hogwords'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'hogwords') : esc_html__('Newest posts', 'hogwords') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'hogwords') : esc_html__('Older posts', 'hogwords'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'hogwords_single_comments_field' ) ) {
    function hogwords_single_comments_field( $args ) {
