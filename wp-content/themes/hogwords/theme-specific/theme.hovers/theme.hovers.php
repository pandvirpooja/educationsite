...
if (!function_exists('hogwords_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'hogwords_hovers_theme_setup3', 3 );
	function hogwords_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		hogwords_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'hogwords'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'hogwords') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'hogwords'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'hogwords'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'hogwords') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
...
if (!function_exists('hogwords_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'hogwords_hovers_theme_setup9', 9 );
	function hogwords_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'hogwords_hovers_frontend_scripts', 1010 );
		add_filter( 'hogwords_filter_localize_script','hogwords_hovers_localize_script' );
		add_filter( 'hogwords_filter_merge_scripts',	'hogwords_hovers_merge_scripts' );
		add_filter( 'hogwords_filter_merge_styles',	'hogwords_hovers_merge_styles' );
		add_filter( 'hogwords_filter_get_css', 		'hogwords_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'hogwords_hovers_frontend_scripts' ) ) {
	
	function hogwords_hovers_frontend_scripts() {
		if ( hogwords_is_on(hogwords_get_theme_option('debug_mode')) && hogwords_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'hogwords-hovers', hogwords_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( hogwords_is_on(hogwords_get_theme_option('debug_mode')) && hogwords_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'hogwords-hovers',  hogwords_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('hogwords_hovers_merge_scripts')) {
	
	function hogwords_hovers_merge_scripts($list) {
...
if (!function_exists('hogwords_hovers_merge_styles')) {
	
	function hogwords_hovers_merge_styles($list) {
...
if (!function_exists('hogwords_hovers_localize_script')) {
	
	function hogwords_hovers_localize_script($arr) {
		$arr['button_hover'] = hogwords_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('hogwords_hovers_add_icons') ) {
	function hogwords_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					hogwords_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'hogwords') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'hogwords');
						else
							esc_html_e('Details', 'hogwords');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'hogwords_hovers_get_css' ) ) {
	
	function hogwords_hovers_get_css($css, $colors, $fonts, $scheme='') {
