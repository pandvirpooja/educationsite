...
while ( have_posts() ) { the_post();
		...
		?><div class="team_member_description"><?php
			...
			<div class="team_member_details">
				...
				foreach ($override_options as $k=>$v) {
                    if (!empty($v['details']) && !empty($meta[$k])) {
                        ?><div class="team_member_details_<?php echo esc_attr($k); ?>"><span class="team_member_details_label"><?php echo esc_html($v['title']); ?>: </span><span class="team_member_details_value"><?php echo wp_kses_post($meta[$k]); ?></span></div><?php
                    }
				}
				?>
			</div>
			...
			if (!empty($meta['brief_info'])) {
				...
				<div class="team_member_brief_info">
					<h5 class="team_member_brief_info_title"><?php esc_attr_e('Brief info', 'hogwords'); ?></h5>
	...
	if ( !trx_addons_sc_layouts_showed('featured') && has_post_thumbnail() ) {
		...
		<div class="team_member_avatar">
			<?php
			the_post_thumbnail( hogwords_get_thumb_size('big-team-avatar'), trx_addons_seo_image_params(array(
