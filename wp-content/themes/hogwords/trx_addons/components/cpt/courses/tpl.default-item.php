...
<div class="sc_courses_item trx_addons_hover trx_addons_hover_style_links">
	<?php if (has_post_thumbnail()) { ?>
		<div class="sc_courses_item_thumb">
			<?php the_post_thumbnail( trx_addons_get_thumb_size($args['columns'] > 2 ? 'huge' : 'big'), array('alt' => the_title_attribute( array( 'echo' => false ) )) ); ?>
			...
			<div class="sc_courses_item_header_desc">
				<div class="sc_courses_item_meta">
					<span class="sc_courses_item_meta_item sc_courses_item_meta_date"><?php
						$dt = $meta['date'];
						echo sprintf($dt < date('Y-m-d') ? esc_html__('Started on %s', 'hogwords') : esc_html__('Starting %s', 'hogwords'), '<span class="sc_courses_item_date">' . date_i18n(get_option('date_format'), strtotime($dt)) . '</span>');
					?></span>
	...
	<div class="trx_addons_hover_content">
		...
		<div class="trx_addons_hover_links">
			<a href="<?php the_permalink(); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'trx_addons_hover_link', 'sc_courses', $args)); ?>"><?php esc_html_e('details', 'hogwords'); ?></a>
			<?php if (!empty($meta['product']) && (int) $meta['product'] > 0) { ?>
			<a href="<?php the_permalink($meta['product']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'trx_addons_hover_link2', 'sc_courses', $args)); ?>"><?php esc_html_e('Buy Now', 'hogwords'); ?></a>
			<?php } ?>
		</div>
	</div>
</div>
