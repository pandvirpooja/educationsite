					...
					?></h3><?php
						...
						if (!empty($item['image'])) {
							...
							if (!empty($item['image'])) {
								...
								?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php echo esc_attr(basename($item['image'])); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
							}
						} else {
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'hogwords_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				if (!empty($item['price'])) {
					...
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'hogwords_kses_content'); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'hogwords_kses_content'); ?></span><?php
						if (count($parts) > 1 && $parts[1]!='') {
							?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'hogwords_kses_content'); ?></span><?php
						}
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'hogwords_kses_content'); ?></span><?php
						}
					?></div><?php
				}
