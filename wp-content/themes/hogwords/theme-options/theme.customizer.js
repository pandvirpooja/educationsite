/* global hogwords_color_schemes, hogwords_dependencies, Color */
...
( function( api ) {
	...
	for (var i in hogwords_color_schemes) {
		cssTemplate[i] = wp.template( 'hogwords-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'hogwords-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+hogwords_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(hogwords_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', hogwords_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', hogwords_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			hogwords_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) hogwords_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		hogwords_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		hogwords_customizer_refresh_preview(obj);
	});

	// Return value of the control
	function hogwords_customizer_get_field_value(fld) {
	...
	function hogwords_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = hogwords_customizer_get_field_value(fld);
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					ctrl.show().removeClass('hogwords_options_no_use');
				} else {
					ctrl.hide().addClass('hogwords_options_no_use');
				}
			}
			...
			if (id == 'color_scheme') {		// Disable color schemes less then main scheme!
				...
				if (fld.length > 0) {
					val = hogwords_customizer_get_field_value(fld);
					var num = 0;
					for (i in hogwords_color_schemes) {
						num++;
						if (i == val) break;
					}
					cont.find('.customize-control').each(function() {
	...
	function hogwords_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in hogwords_theme_fonts) {
					for (rule in hogwords_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							hogwords_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) hogwords_customizer_update_css();
		}
	...
	// Actions buttons
	function hogwords_customizer_actions(e) {
	...
	function hogwords_customizer_update_theme_fonts(opt, rule, value) {
		hogwords_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function hogwords_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= hogwords_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in hogwords_theme_fonts) {
	...
	function hogwords_customizer_update_css() {
		...
		var fonts = hogwords_customizer_add_theme_fonts(hogwords_theme_fonts);

		// Make styles and add into css
		css += hogwords_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in hogwords_color_schemes) {
			
			var i, colors = [];
			
			// Copy all colors!
			for (i in hogwords_color_schemes[scheme].colors) {
				colors[i] = hogwords_color_schemes[scheme].colors[i];
			}
            ...
            var tmpl = jQuery( '#tmpl-hogwords-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function hogwords_customizer_add_theme_colors(colors) {
		if (hogwords_additional_colors) {
			var clr = '', v = '';
			for (var k in hogwords_additional_colors) {
				v = hogwords_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + hogwords_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + hogwords_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + hogwords_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function hogwords_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function hogwords_customizer_prepare_html_value(val) {
