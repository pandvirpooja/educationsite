...
jQuery(document).ready(function() {
	...
	function trx_addons_ready_actions(e, container) {
		...
		// Likes counter
			//---------------------------------------------
			var $likes = container.find('a.post_counters_likes:not(.inited),a.comment_counters_likes:not(.inited)');
			if ($likes.length > 0) {
						...
						if ( ! likes_busy) {
							...
							jQuery.post( TRX_ADDONS_STORAGE['ajax_url'], {
								...
								if (rez.error === '') {
									...
									trx_addons_set_cookie( button.hasClass('post_counters_likes') ? 'trx_addons_likes' : 'trx_addons_comment_likes', cookie_likes, 365 * 24 * 60 * 60 * 1000);
								} else {
